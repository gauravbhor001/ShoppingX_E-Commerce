...
class ProductView(View):
    def get(self, request):
        ...
        product1 = Product.objects.filter(category='L')
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
        return render(request, 'app/home.html',
                      {'topwears': topwears, 'bottomwears': bottomwears, 'mobiles': mobiles, 'product1' :product1,'totalitem': totalitem})
...
class guitar(View):
    def get(self, request):
        ...
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
        return render(request, 'app/guitar.html',
                      {'topwears': topwears,'totalitem': totalitem})
...
class piano(View):
    def get(self, request):
        ...
        if request.user.is_authenticated:
            totalitem = len(Cart.objects.filter(user=request.user))
        return render(request, 'app/piano.html',
